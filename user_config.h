...
//#define Reset_Every6Hours						 // uncomment to resert module every six hours (timers don't exced this)
#define USE_InterlockDelay						 // comment to not use interlock delay (delay between off and on relay on interlock action)
#define USE_PowerEvenOdd						 // use with more than 2 relays to switch( toggle) even relays or odd relays (until 4 relays) use : 10 = on even (1 then 3), 11 = on odd (2 then 4)
												 // the second (3 or 4)  one  will begin (on/off) when the first (1 or 2) will done off , mainly use with intrelock and pulsetimer on each
...
#define STA_SSID1              "reseau1"      // [Ssid1] Wifi SSID
#define STA_PASS1              "1234567891234"  // [Password1] Wifi password
#define STA_SSID2              "reseau2"      // [Ssid2] Optional alternate AP Wifi SSID
...
#define WIFI_CONFIG_TOOL       WIFI_MANAGER    // [WifiConfig] Default tool if wifi fails to connect
	...
	// -- Syslog --------------------------------------
	//#define Use_Syslog						// uncomment to use_syslog (+1 K) add cpat
#ifdef Use_Syslog // add cpat
	#define SYS_LOG_HOST           "pc-ras"          // [LogHost] (Linux) syslog host
...
// module and other for increase free memory
// add cpat
// #define ExtraSonOff uncomment to add extra sonoff modules  Memory  1K
//#define USE_ElectroDragon //  comment to not use electroradgon add cpat
							// if you use electrodragon set flash size to 4Mo (1 Mo SPIFFS) cause in this case it use fs.h library 

//see also use_emulation, use_relay,... (in web server section) bellow

// -- Ota -----------------------------------------
//#define Use_WebOta						// uncomment to use_webota (+1.5K) add cpat
#ifdef Use_WebOta
	#define OTA_URL                "http://domus1:80/api/arduino/" PROJECT ".ino.bin"  // [OtaUrl]
											// see also bellow use of ota_url as send off message
#endif
...
#else
  #define MQTT_HOST            "mosquito"          // [MqttHost]
...
//#define Use_DNSServeur
 ...
 // #define USE_EMULATION                          // Enable Belkin WeMo and Hue Bridge emulation for Alexa (+11k code, +2k mem)
  ...
  #define Use_Information					    // uncomment to have an info page (+4K) add cpat
	
	// see also use_webota upper
// -- mDNS ----------------------------------------
...
#define USE_ADC_MAX			 // add cpta     	   // define value of adc read not as an average (32 reads) = default setting  but like the max of the 32 reading (alterntive wave)
													// if turn On set an other message for read adc and others commande to set Current resolution (coef/10) and current maxi before off
													// if set when current adc value is over i_max_tooff settings all the output is set to off
													// if set then current value of adc is in i_max settings (witch is save)
#define Use_HLW8012							// uncomment to use hlw8012 library  (sonoff pow,kuan sf, kmc)
#define USE_LED								// uncomment to use device with led and pvm (sonoff led, sonoff bn, h801, arilux, magichome, witty
...
#ifdef USE_I2C 
	#define USE_SSD1306 // comment to not use SSD1306 (194*64 oled display) via I2C
#endif
