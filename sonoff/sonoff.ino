...
#include "xcpat.h"
#define VERSION                0x05090200   // 5.9.2 modif add cpat
...
#include <ArduinoJson.h>                    // WemoHue, IRremote, Domoticz
  ...
  #include <Wire.h>                         // I2C support library
	#ifdef USE_SSD1306
	// add cpat use oled display type ssd1306 
...
#ifdef USE_SSD1306
    // add cpat use oled display type ssd1306 
    //#ifdef USE_ElectroDragon // add cpat
...
// Structs
#include "settings.h"
// global use to freememory
const char kDecim[] PROGMEM = "%d";
const char kString[] PROGMEM = "%s";
const char kStringComa[] PROGMEM = "%s, ";
const char kStringSlach[] PROGMEM = "%s\"}";
const char kStringQuote[] PROGMEM = "%s}";
...
enum TasmotaCommands {
  ...
  CMND_CFGDUMP, CMND_I2CSCAN, CMND_OUT, CMND_EXCEPTION };
const char kTasmotaCommands[] PROGMEM =
  ...
  D_CMND_CFGDUMP "|" D_CMND_I2CSCAN "|" D_CMND_OUT
  #ifdef DEBUG_THEO
  "|" D_CMND_EXCEPTION
#endif
  ;
...
const char kCmnd[] PROGMEM = D_CMND;
#ifdef USE_ElectroDragon // add cpat
...
// add cpat
	...
	uint16_t init_counter = 21600;			   // counter for restart wifi every 6 hours add cpat
#endif
#ifdef USE_InterlockDelay // add cpat
	boolean is_SetToZero = false;         // set if interlock off is in progress add cpat
	uint16_t interlockOff_Delay = 0;		// current interlockoff delay
	byte interlockOff_DeviceOn = 0;			// current stand by on device during interlock
#endif
#ifdef USE_PowerEvenOdd						// if use even and odd synchro. on relay
	uint8 EvenOddMode = 0;					// Mode Even Odd relays in progress when the first (1 or 2) on is down (off) , the second one (3 or 4) will begin
											// 0 = not in progress, 1= Even in progress , 2 = odd in progress
#endif 
#ifdef USE_MQTT_TLS
...
//const int16_t kSizeMqttData PROGMEM = MESSZ; // to reduce size of prog
...
*/
	...
	#ifdef USE_PowerEvenOdd // add cpat
		else if ((CMND_POWER == command_code) && (index > 10) && (index <= 14)) {
				ExecuteCommandPower(index-10, 1);
				EvenOddMode = index - 10;
				fallback_topic_flag = 0;
				return;
		}
		else if ((CMND_POWER == command_code) && (index > 20) && (index <= 24)) {
			// to block timer on
			//todo ExecuteCommandPower(index - 20, 1);
			//todo EvenOddMode = index - 10;
			//todo fallback_topic_flag = 0;
			return;
		}
	#endif
    ...
    else if ((CMND_SETOPTION == command_code) && ((index >= 0) && (index <= 17)) || ((index > 31) && (index <= P_MAX_PARAM8 +31)) || (index==64)) {
      if (index <= 31) {
          ...
          switch (index) {
            ...
            case P_MAX_POWER_RETRY:
              ...
              break;
				...
				case 32:
					//delai interlock
					Settings.delay_InterlockOff = payload16;
					/* debug snprintf_P(log_data, sizeof(log_data), PSTR(D_LOG_RESULT " OPT18 %d, " D_INDEX " %d, " D_COMMAND " %d, " D_DATA " %d"),
						index, type , payload, payload16); 
					AddLog(LOG_LEVEL_DEBUG); */
					break;
			#endif
          }
      ...
      if (ptype)  {
		  if (index > 31) {
			...
			#ifdef USE_InterlockDelay
			  case 32:
				  snprintf_P(stemp1, sizeof(stemp1), kDecim, Settings.delay_InterlockOff);
				break;
			#endif
			  default:
				snprintf_P(stemp1, sizeof(stemp1), PSTR(D_UNKNOWN));
				break;
			  }
		  }
		  else {
			  // option32..63
			  snprintf_P(stemp1, sizeof(stemp1), kDecim, Settings.param[index]);
		  }
      }
      ...
      snprintf_P(mqtt_data, sizeof(mqtt_data), S_JSON_COMMAND_NVALUE, command, Settings.flag.temperature_resolution);
		...
		#else
			if ((payload >= 0) && (payload <= 1204)) {
				Settings.i_max_to_off = payload;
			}
			snprintf_P(mqtt_data, sizeof(mqtt_data), S_JSON_COMMAND_NVALUE, command, Settings.i_max_to_off);
			// add cpat set the maxi current before off (in adc resolution value (0 to 1024))
		#endif
    }
		...
		#else
			// in this case the keyword is not 'PressRes' but 'CurrRes' see language value
			// value of coef /10 apply to adc value for visu current in mA (both current value and maxi current )
			// re-use of ws_red setting , not use in this version
			// coef can be use as dividor if < 10 or multiplicator if > 10
			if ((payload >= 0) && (payload <= 255)) {
				Settings.ws_red = payload;
			}
			snprintf_P(mqtt_data, sizeof(mqtt_data), S_JSON_COMMAND_NVALUE, command, Settings.ws_red);
		#endif
	}
      ...
      for (byte i = 0; i < MAXMODULE; i++) {
        if (!jsflg) {
          ...
          snprintf_P(mqtt_data, sizeof(mqtt_data), kStringComa, mqtt_data);
        }
        jsflg = 1;
        ...
        if ((strlen(mqtt_data) > 200) || (i == MAXMODULE -1)) {
          snprintf_P(mqtt_data, sizeof(mqtt_data), kStringSlach, mqtt_data);
      ...
      if (jsflg) {
        snprintf_P(mqtt_data, sizeof(mqtt_data), kStringQuote, mqtt_data);
      } else {
...
//          snprintf_P(mqtt_data, sizeof(mqtt_data), PSTR("%s after restart"), mqtt_data);
	...
	else if ((CMND_OUT == command_code) && (index > 0) && (index <=8)) {
		//	sprintf(Settings.Outtimer[index], "%s", dataBuf);
		// debug AddLog_P(LOG_LEVEL_INFO, "init out");
		if ((data_len > 0) && (data_len < 15)) {
			strlcpy(Settings.Outtimer[index-1], (1 == payload) ? "" : dataBuf, 15);
			
		}
		snprintf_P(mqtt_data, sizeof(mqtt_data), S_JSON_COMMAND_INDEX_SVALUE, command, index, Settings.Outtimer[index - 1]);
	}
#ifdef USE_I2C
...
// state 9 = Show power state
  ...
  if (state <= 2) {
      ...
      for (byte i = 0; i < devices_present; i++) {
        ...
        if ((power & imask) && (mask != imask)) {
          ExecuteCommandPower(i +1, 0);
			#ifdef USE_InterlockDelay //add cpat
				is_SetToZero = true; interlockOff_DeviceOn = device;
			#endif
		   }
		}
		interlock_mutex = 0;
		#ifdef USE_InterlockDelay //add cpat
			if ((is_SetToZero) && (state != 0) && (Settings.delay_InterlockOff>100)) {
				// to prevent set at 1
				//AddLog_P(LOG_LEVEL_INFO, "set time interlock");
				interlockOff_Delay = Settings.delay_InterlockOff;
				return;
			}
		#endif
    }
    ...
    case 0: { // Off
		...
		#ifdef USE_PowerEvenOdd // add cpat
			if ((EvenOddMode > 0) && (EvenOddMode == device)) {
				if (devices_present > device + 1)	 {
					// next device is present
					#ifdef USE_InterlockDelay //add cpat
						if (Settings.flag.interlock) {
							is_SetToZero = true;
							interlockOff_DeviceOn = device + 2;
							interlockOff_Delay = Settings.delay_InterlockOff;
						} else { ExecuteCommandPower(device + 2, 1);	}
					#else
						ExecuteCommandPower(device + 2, 1);
					#endif
				}
				else {
				
				}
			}// end if evendevice=device
			else {
				if ((EvenOddMode > 0) && ((EvenOddMode -10) == device)) {
					
					
				}
			} // end if evendevice-10 = device
	...
	#ifdef USE_ADC_MAX
		if ((bitRead(power, device - 1) == 1) && (!blink_timer)) {
			blink_timer = 20; // set to 1.5 s to wait before test adc (current) and test with i_max_to_off value 
			}
	#endif
	#ifdef USE_SENDOFF
		if (bitRead(power, device - 1) == 0) {
			//set to off
			// send an http command to another tasmota or an http server
			//AddLog_P(LOG_LEVEL_INFO, PSTR("Try to send URL") );
			sendget(device - 1);
		}
	#endif
	#ifdef USE_InterlockDelay // add cpat
		if ((state != 0) && (is_SetToZero))  { is_SetToZero = false; }
	#endif
#ifdef USE_DOMOTICZ
...
void sendget(byte dev) {
	// send a message to otaurl via hhhtp get
	// Use WiFiClient class to create TCP connections
	if ((Settings.ota_url[0]=='\0') || (dev>7) || (dev<0)) {
		// AddLog_P(LOG_LEVEL_INFO, PSTR("url empty"));
		snprintf_P(log_data, sizeof(log_data), PSTR("%d Can't Send at '%s'"), dev,Settings.ota_url ) ;
		AddLog(LOG_LEVEL_INFO);
		return; }
	if (Settings.Outtimer[dev,0]=='\0'){
		snprintf_P(log_data, sizeof(log_data), PSTR("%d Nothing to Send at '%s'"), dev, Settings.ota_url);
		AddLog(LOG_LEVEL_INFO);
		return;
	}
	WiFiClient client;
	...
	if (!client.connect(Settings.ota_url, httpPort)) {
		snprintf_P(log_data, sizeof(log_data), PSTR("%d Connect Fail '%s'"), dev, Settings.ota_url);
		AddLog(LOG_LEVEL_INFO);
		return;
	}
	// We now create a URI for the request
	
	String url =  Settings.Outtimer[dev];
	url.replace(" ", F("%20"));
	
	// This will send the request to the server
	client.print(String("GET /" + url + " HTTP/1.1\r\nHost:" + Settings.ota_url + "\r\nConnection: close\r\n\r\n"));
	
	unsigned long timeout = millis();
	while (client.available() == 0) {
		if (millis() - timeout > 5000) {
			snprintf_P(log_data, sizeof(log_data), PSTR("%d Connect time out at '%s', '%s'"), dev, Settings.ota_url, Settings.Outtimer[dev]);
			AddLog(LOG_LEVEL_INFO);
			client.stop();
			return;
		}
	...
	//Serial.println();
	snprintf_P(log_data, sizeof(log_data), PSTR("%d:%s sent at '%s'"), dev, Settings.Outtimer[dev], Settings.ota_url);
	AddLog(LOG_LEVEL_INFO);//Serial.println("closing connection");
...
void PublishStatus(uint8_t payload)
{
  ...
  if ((0 == payload) || (4 == payload)) {
    snprintf_P(mqtt_data, sizeof(mqtt_data), PSTR("{\"" D_CMND_STATUS D_STATUS4_MEMORY "\":{\"" D_PROGRAMSIZE "\":%d, \"" D_FREEMEMORY "\":%d, \"" D_HEAPSIZE "\":%d, \"" D_PROGRAMFLASHSIZE "\":%d, \"" D_FLASHSIZE "\":%d, \"" D_FLASHMODE "\":%d, \"" D_FREEMEMORY "\":%d}}"),
		ESP.getSketchSize() / 1024, ESP.getFreeSketchSpace() / 1024, ESP.getFreeHeap() / 1024, ESP.getFlashChipSize() / 1024, ESP.getFlashChipRealSize() / 1024, ESP.getFlashChipMode(), ESP.getFreeHeap());
...
void PerformEverySecond()
{
  ...
  CP_PerformEverySecond(); //add cpat lib
} // end every second sub
...
*/
	...
	#ifdef USE_InterlockDelay
		if (is_SetToZero) {
			//interlock off in progress
			interlockOff_Delay -= 100;
			if (interlockOff_Delay <= 0) {
				// debug AddLog_P(LOG_LEVEL_INFO, "raz time interlock");
				// execute stand by power on 
				is_SetToZero = false;
				interlockOff_Delay = 0;
				ExecuteCommandPower(interlockOff_DeviceOn, 1);
			}
		}
	#endif
  } //end state/10 every .01 sec
...
void setup()
{
  ...
  if (Serial.baudRate() != baudrate) {
    ...
    Serial.begin(baudrate);
    delay(10);
    Serial.println("U"); //add cpat use to oled serial dislay to autobaudrate
  }
  ...
  RtcInit();
		  ...
		  if (!SPIFFS.begin())
		  {
			  ...
			  //SPIFFS.format();
			  ftpSrv.begin(F("esp8266"), F("esp8266"));
			 // copie file1 dans le ssfis
		  }
    #endif
	#ifdef USE_SSD1306
		// add cpat use oled display type ssd1306 
		...
		OledDis.flipScreenVertically();
		OledDis.setFont(ArialMT_Plain_16);
		OledDis.setTextAlignment(TEXT_ALIGN_LEFT);
		OledDis.clear();
		OledDis.drawString(0, 0, F("Init. C.Pat."));
		OledDis.display();
