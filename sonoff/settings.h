...
struct SYSCFG {
  ...
  uint8_t       i_max;                     // 2FA ex_power Not used since 5.8.0j //use add cpat to content current value of adc max read 
  ...
  uint16_t      timeOnAdc;                 // 39E ws_pixels Not used since 5.8.0 // re-use to store time before 'on' relay to test adc (currrent) value 
  uint8_t       ws_red;                    // 3A0 Not used since 5.8.0 // re-use in version 5.9.2 add cpat to strore current divisor when use adc current sensor
  ...
  uint16_t      i_max_to_off;                 // 3AA Not used since 5.8.0 use now Add cpat to set max limit of adc input to stop all power on
  ...
  uint16_t		delay_InterlockOff;		   // 5D5 add cpat delay between off and on when interlock
  uint16_t      pulseon_timer[MAX_PULSETIMERS]; // 5D7  add cpat time to on when clock function (time off set by pulse_timer upper)
  char			Outtimer[MAX_PULSETIMERS][30];		   // 5C7 add cpat message send when off on each relay by http get client on ota_web server (set upper)
} Settings;
// temporaire avant d'introduire dans setting si besoin 
