...
#if #define(Use_WebOta) || #define(USE_SENDOFF)  
...
//  Settings.altitude = 0;
  ...
  // 5.9.3
  // setting Cpat
  Settings.delay_InterlockOff = 0;
  SettingsDefaultSet_5_9_2();
  //Settings.pw
}
...
{
  Settings.timeOnAdc = 30;  // ws_pixels settngs re-use add cpat to stote time on before test old value =  WS2812_LEDS;
  Settings.ws_red = 255;
  Settings.ws_green = 0;
  Settings.ws_blue = 0;
...
void SettingsDefaultSet_5_9_2()
{
	//  Settings.sendmessage ;
	for (byte i = 0; i < MAX_PULSETIMERS; i++) {
		Settings.pulseon_timer[i] = 0;
		strlcpy(Settings.Outtimer[i] ,"",30);
	}
}
...
//      Settings.flag.mqtt_button_retain = 0;
      ...
      uint8_t cfg_wsflg = 0;
		...
		//#ifdef USE_LED
			  if (!Settings.light_pixels && cfg_wsflg) {
				  Settings.light_pixels = 30; // re-use add cpat to store time since on before test adc (current) old Settings.ws_pixels;
				...
				Settings.light_wakeup = 0;// add cpat re-user of Settings.ws_wakeup in i_max_to_off;
			  } else {
				Settings.light_pixels = WS2812_LEDS;
				Settings.light_width = 1;
			  }
    ...
    if (Settings.version < 0x0508000A) {
	  Settings.power = 0; // add cpat re use ex_power setting Settings.ex_power in i_max;
      Settings.altitude = 0;
    }
	...
	if (Settings.version < 0x0509020E) {
		SettingsDefaultSet_5_9_2();
	}
    Settings.version = VERSION;
    SettingsSave(1);
