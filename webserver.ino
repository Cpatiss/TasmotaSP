...
"</script>"
"<style>"
"div,fieldset,input,select{padding:1px;font-size:1em;}"
"input{width:100%;box-sizing:border-box;-webkit-box-sizing:border-box;-moz-box-sizing:border-box;}"
"select{width:100%;}"
"textarea{resize:none;width:98%;height:318px;padding:3px;overflow:auto;}"
...
#endif
	"<div style='text-align:center;'><h1>C.P.{ha<br/>{h}</h1></div>";
...
const char HTTP_BTN_MENU2[] PROGMEM =
  "<form action='md' method='get'><button>" D_CONFIGURE_MODULE "</button></form>"
...
const char HTTP_BTN_MENU4[] PROGMEM =
  "<br/><form action='lg' method='get'><button>" D_CONFIGURE_LOGGING "</button></form>";
	...
	#ifdef USE_ElectroDragon
		const char HTTP_BTN_MENU5[] PROGMEM =
			"<br/><form action='c2' method='get'><button>" D_CONFIGURE_RELAY "</button></form>"
			"<br/><form action='index.html' method='get'><button> Pge </button></form>";
	#else
		const char HTTP_BTN_MENU5[] PROGMEM =
		"<br/><form action='c2' method='get'><button>" D_CONFIGURE_RELAY "</button></form>";
	#endif
#endif
const char HTTP_BTN_MENU6[] PROGMEM =
...
const char HTTP_BTN_MAIN[] PROGMEM =
	"<br/><form action='.' method='get'><button>" D_MAIN_MENU "</button></form>";
...
const char HTTP_FORM2_WIFI[] PROGMEM =
	"<fieldset><form method='get' action='ip0'>"
	"<b>" D_IP_ADDRESS "</b> (" WIFI_IP_ADDRESS ")<br/><input id='setip' name='setip' placeholder='" WIFI_IP_ADDRESS "' value='{ip1'><br/>"
	"<br/><button type='submit'>" D_ACTIVE "</button></form></fieldset>";
	//"<br/><b>" D_SUBNET_MASK "</b> (" WIFI_SUBNETMASK ")<br/><input id='sn' name='h' placeholder='" WIFI_SUBNETMASK "' value='{sn1'><br/>"
	//"<br/><b>" D_GATEWAY "</b> (" WIFI_GATEWAY ")<br/><input id='h' name='gw' placeholder='" WIFI_GATEWAY "' value='{gw1'><br/>";
...
"<input id='c1' name='c1' placeholder='" D_ENTER_COMMAND "' value='{in1'autofocus><br/>"
//  "<br/><button type='submit'>Send command</button>"
"</form>";

/*<br / ><table style = 'width:100%'><tr>"
  "<td style='width:25%'><form action='st1' method='get'><button style='width:50%;'>" D_CMND_STATUS "</button></form></td>"
  "<td style='width:25%'><form action='ip1' method='get'><button style='width:50%;'>" D_IP_ADDRESS "</button></form></td>"
  "<td style='width:25%'><form action='gw1' method='get'><button style='width:50%;'>" D_GATEWAY "</button></form></td>"
  "</tr></table>";
  */
...
void StartWebserver(int type, IPAddress ipweb)
{
...
#ifdef Use_Relay
	WebServer->on("/c2", HandleRelayConfiguration);
#endif
	WebServer->on("/ip0", HandleAddIpConfig0);
#ifdef USE_WEBSERVER // add cpat
	#ifdef USE_ElectroDragon
		(*WebServer).serveStatic("/img", SPIFFS, "/");
  ...
  if (webserver_state != type) {
    snprintf_P(log_data, sizeof(log_data), PSTR(D_LOG_HTTP D_WEBSERVER_ACTIVE_ON " %s%s " D_WITH_IP_ADDRESS " %s %d"),
      my_hostname, (mdns_begun) ? ".local" : "", ipweb.toString().c_str(), type);
    AddLog(LOG_LEVEL_INFO);
	#ifdef USE_SSD1306
		OledDis.clear();
		OledDis.drawString(0, 0, F("Web On C.Pat."));
		OledDis.drawString(0, 16, ipweb.toString().c_str());
		OledDis.drawString(0, 32, my_hostname);
		OledDis.display();
		WebStarted = 1;
	#endif
  }
...
#ifdef Use_DNSServeur
  DnsServerrrorReplyCode(DNSReplyCode::NoError);
  ...
  if (HTTP_MANAGER == webserver_state) {
    ...
    if (devices_present) {
      ...
      for (byte idx = 1; idx <= devices_present; idx++) {
		  ...
		  if( (Settings.module == SONOFF_DUAL2) || (Settings.module == SONOFF_DUALC) || (Settings.module == ELECTRODRAGON)) {
			if (idx == 1) {
				snprintf_P(stemp2, sizeof(stemp2), kString, D_BUTTON_UP);
			}
			else { snprintf_P(stemp2, sizeof(stemp2), kString, D_BUTTON_DOWN); }
			idx2 = (idx+1)/2;
			}
			
		else
		{
			snprintf_P(stemp2, sizeof(stemp2), kString, D_BUTTON_TOGGLE);
			
		}
		snprintf_P(stemp, sizeof(stemp), PSTR(" %d"), idx2);
       snprintf_P(line, sizeof(line), PSTR("<td style='width:%d%'><button onclick='la(\"?o=%d\");'>%s%s</button></td>"),
				100 / devices_present, idx, (devices_present < 5) ? stemp2 : "", (devices_present > 1) ? stemp : "");
        page += line;
      }
      page += F("</tr></table>");
    }
    ...
    if (HTTP_ADMIN == webserver_state) {
	  ...
	  page.replace(F("{dt"), ""); //  GetBuildDateAndTime().substring(5, 10));
...
{
	if ((Settings.module == SONOFF_LED) || (Settings.module == SONOFF_SC) || (Settings.module == SONOFF_BN) || (Settings.module == SONOFF_BRIDGE)
		|| (Settings.module == SONOFF_B1) || (Settings.module == SONOFF_DEV)) {
		With_Relay = false;
	}
#ifdef ExtraSonOff 
	if ((Settings.module == ELECTRODRAGON) || (Settings.module == WEMOS) || (Settings.module == H801) || (Settings.module == AILIGHT)
		|| (Settings.module == WITTY) || (Settings.module == MAGICHOME) || (Settings.module == ARILUX)) {
		With_Relay = false;
	}
#endif
  if (HttpUser()) {
...
void HandleWifi(boolean scan)
{
  ...
  //char stemp[15];
  //snprintf_P(stemp, sizeof(stemp), PSTR("%d.%d.%d.%d"), Settings.ip_address[0], Settings.ip_address[1], Settings.ip_address[2], Settings.ip_address[3]);
  
  //page.replace(F("{sn1"), IPAddress(Settings.ip_address[2]).toString());
  //page.replace(F("{gw1"), IPAddress(Settings.ip_address[1]).toString());
  page += FPSTR(HTTP_FORM_END);
  page += FPSTR(HTTP_FORM2_WIFI);
  page.replace(F("{ip1"), IPAddress(Settings.ip_address[0]).toString());
  if (HTTP_MANAGER == webserver_state) {
    page += FPSTR(HTTP_BTN_RSTRT);
  } else {
    page += FPSTR(HTTP_BTN_CONF);
...
void HandleSaveSettings()
{
  ...
  case 1:
	  ...
	  /*char dataBuf[15];
	  uint32_t address;
	     snprintf_P(dataBuf, sizeof(dataBuf), kString, WebServer->arg("ip").c_str());
	     if (ParseIp(&address, dataBuf)) {
		 	  Settings.ip_address[0] = address;
			  //        restart_flag = 2;
	//		  snprintf_P(log_data, sizeof(log_data), PSTR("Change ip adresse %s %d"), dataBuf, address);
	//		  AddLog(LOG_LEVEL_INFO);
	   }*/
...
void HandleUploadDone()
{
  ...
  AddLog_P(LOG_LEVEL_DEBUG, PSTR(D_LOG_HTTP D_UPLOAD_DONE));
	  ...
	  OledDis.clear();
	  WebStarted = 0;
	  OledDis.drawString(0,0,F("Uplaod OK"));
	  OledDis.drawString(0,16,F("Reset in progress"));
	  OledDis.display();
...
void HandleUploadLoop()
{
  ...
  if (UPLOAD_FILE_START == upload.status) {
    ...
    AddLog(LOG_LEVEL_INFO);
		...
		WebStarted = 0;
		OledDis.clear();
		OledDis.drawString(0,0,F("Uplaod"));
		OledDis.drawString(0,16,upload.filename.c_str());
		OledDis.display();
...
void HandleHttpCommand()
{
	...
	if ((HttpUser()) && (strcmp(WebServer->arg("user").c_str(), WEB_USERNAME))) {
    return;
  }
  char svalue[INPUT_BUFFER_SIZE];  // big to serve Backlog
  
  snprintf_P(messageb, sizeof(messageb), PSTR("u:%s, p:%s, a%s"), WebServer->arg("user").c_str(), WebServer->arg("password").c_str(), Settings.web_password );
  AddLog_P(LOG_LEVEL_INFO, messageb);
  ...
  if (valid) {
	...
	if (strlen(WebServer->arg(kCmnd).c_str())) {
//      snprintf_P(svalue, sizeof(svalue), WebServer->arg("cmnd").c_str());  // Processes FullTopic %p
		strlcpy(svalue, WebServer->arg(kCmnd).c_str(), sizeof(svalue));       // Fixed 5.8.0b
...
void HandleConsole()
{
  ...
  page.replace(F("{in1"), currentHelp);
  page += FPSTR(HTTP_BTN_MAIN);
  ShowPage(page);
}
...
//    syslog_level = syslog_now;
  ...
  WebServer->send(200, FPSTR(HDR_CTYPE_XML), message);
	...
	void HandleIndex() {
		...
		Serial.println(F("deb2"));
		AddLog_P(LOG_LEVEL_DEBUG, S_LOG_HTTP, "Index");
		//String page = FPSTR(HTTP_BTN_MAIN);
		...
		// else
		// {
		//	 f.println(FPSTR(File1));
		// }
		//f.close();
		
		//Serial.println(FPSTR(File1));

		WebServer->send(200, FPSTR(HDR_CTYPE_HTML),"/indx");

	}// fin void handle index
#endif
#ifdef Use_Information
...
void HandleAddIpConfig0()
{
	/*snprintf_P(currentHelp, sizeof(currentHelp), PSTR("IPAddress1 "));
	'HandleConsole();
	'snprintf_P(currentHelp, sizeof(currentHelp), PSTR(""));*/
	char dataBuf[15];
	uint32_t address;
	snprintf_P(dataBuf, sizeof(dataBuf), PSTR("%s"), WebServer->arg("setip").c_str());
	if (ParseIp(&address, dataBuf)) {
	Settings.ip_address[0] = address;
	//        restart_flag = 2;
	//		  snprintf_P(log_data, sizeof(log_data), PSTR("Change ip adresse %s %d"), dataBuf, address);
