...
#ifndef ExtraSonOff  // add cpat
	// to set value at none declare def
	#define S20 -1
	#define SLAMPHER -1
	#define CH1 -1
	#define CH4 -1
	#define MOTOR -1
	#ifndef USE_ElectroDragon // add cpat
		#define ELECTRODRAGON -1
	#endif
	#define EXS_RELAY -1
	#define WION -1
	#define WEMOS -1
...
const mytmplt kModules[MAXMODULE] PROGMEM = {
  ...
  { "Sonoff T1 3CH",   // Sonoff T1 3CH (ESP8285)
	 ...
	 ,{ "Sonoff Dual R2",     // Sonoff Dual (ESP8266)
		 GPIO_KEY1,
		 GPIO_USER,         // GPIO01 Relay control
		 0,
		 GPIO_USER,         // GPIO03 Relay control
		 0,					// GPIO04 Optional sensor
		 GPIO_REL2,
		 0, 0, 0, 
		 GPIO_KEY2	 ,
		 GPIO_SWT1, 0, // Flash connection
		 GPIO_REL1,
		 GPIO_LED1_INV,    // GPIO13 Blue Led (0 = On, 1 = Off)
		 0,        // GPIO14 Optional sensor
		 0, 0, 0
	 }
	 ,{ "Sonoff Dual CP",     // Sonoff Dual (ESP8266)
		 GPIO_KEY1,
		 GPIO_USER,         // GPIO01 Relay control
		 0,
		 GPIO_USER,         // GPIO03 Relay control
		 ...
		 GPIO_LED1_INV,    // GPIO13 Blue Led (0 = On, 1 = Off)
		 0,        // GPIO14 Optional sensor
		 0, 0, GPIO_ADC0 // add cpat sensor TI
	 }
#ifdef ExtraSonOff // add cpat
  ...
  // I am not sure but i think must comment with comiplation directive #ifdef USE_LED cause PVM
#ifdef USE_LED
