...
*/
  if (1 == type) {
    char svalue[10];
    snprintf_P(svalue, sizeof(svalue), kString, 7);  // Exception 28 as number in string (7 in excvaddr)
  }
/*
...
char* GetPowerDevice(char* dest, uint8_t idx, size_t size, uint8_t option)
{
  ...
  if ((devices_present + option) > 1) {
	  snprintf_P(sidx, sizeof(sidx), kDecim, idx);
...
void WifiCheck(uint8_t param)
{
  ...
  default:
    if (wifi_config_counter) {
      ...
      if (wifi_counter <= 0) {
        AddLog_P(LOG_LEVEL_DEBUG, S_LOG_WIFI, PSTR(D_CHECKING_CONNECTION));
        wifi_counter = WIFI_CHECK_SEC;
        WifiCheckIp();
      }
...
void AdcShow(boolean json)
{
  ...
  if (json) {
    snprintf_P(mqtt_data, sizeof(mqtt_data), PSTR("%s, \"" D_ANALOG_INPUT "0\":%d"), mqtt_data, analog);
			...
			#ifdef USE_ADC_MAX
				snprintf_P(mqtt_data, sizeof(mqtt_data), HTTP_SNS_ANALOG, mqtt_data, "",  analog*Settings.ws_red/10, Settings.i_max_to_off*Settings.ws_red/10);
			#else
