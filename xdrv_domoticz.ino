...
void MqttPublishDomoticzPowerState(byte device)
{
  ...
  if (Settings.flag.mqtt_enabled && Settings.domoticz_relay_idx[device -1]) {
	  snprintf_P(sdimmer, sizeof(sdimmer), kDecim, Settings.light_dimmer);
    ...
    if ((idx > 0) && (nvalue >= 0) && (nvalue <= 2)) {
      ...
      for (byte i = 0; i < maxdev; i++) {
        if (idx == Settings.domoticz_relay_idx[i]) {
			snprintf_P(stemp1, sizeof(stemp1), kDecim, i + 1);
          if (2 == nvalue) {
			...
			snprintf_P(dataBuf, sdataBuf, kDecim, nvalue);
            found = 1;
          } else {
            if (((power >> i) &1) == nvalue) {
              return 1;
...
{
  char data[16];
  snprintf_P(data, sizeof(data), kDecim, value);
